...
newConfigs tm c@(Config st _ (l : leftrev) (right)) =
  let nexts = [(st'', g'', d) | Trans st' g' d st'' g'' <- (trans tm), st == st', l == g']
   in let newConfig (st, g, d) = updateConfig c st g d
       in map newConfig nexts
...
acceptsh tm (c@(Config st _ _ _) : cs) =
  if elem st (final tm)
    then True
    else acceptsh tm (cs ++ newConfigs tm c)

initialConfig :: TM input state tape -> [input] -> Config state tape
initialConfig tm xs =
  let t = map (totape tm) xs ++ repeat (blank tm)
   in Config (start tm) (blank tm) [head t, leftend tm] (tail t)
